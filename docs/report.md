...
После PR #33 в `main` было 37 commits: 30 обычных содержательных и 7 merge
commits. Обычные subjects следуют Conventional Commits (`feat`, `fix`, `test`,
`docs`, `ci`, `build`, `security`, `chore`, `refactor`). GitHub-generated merge
subjects сохранены, потому что они доказывают topology реальных PR.
...
Создано 15 GitHub issues, включая bootstrap, domain/storage/CRUD, bug,
unit/integration tests, CI, security, hooks, docs, два releases и отдельное
исследование. Новые issues имеют labels, assignee, описание и acceptance
...
До документационной ветви слиты шесть PR: #3, #16, #30, #31, #32 и #33.
Каждый крупный слой прошёл отдельным PR с checks. В PR #30 опубликован inline
...
Release запускается только по `v*` tag или документированному `release:` merge
...
После security/hooks PR собрано 58 test cases: 48 в `tests/unit`, 9 CRUD API
integration и 1 health smoke. GitHub run #27 показал 97.39% branch coverage;
...
Конфликт 2 появляется при синхронизации независимо развивавшихся docs и
...
`v1.0.0` фиксирует CRUD baseline, SQLite, tests, CI/security/hooks и документацию.
`v1.1.0` добавляет filters/statistics. Версия меняется в `pyproject.toml`,
changelog обновляется до merge, а wheel/sdist берутся из того же successful
run, который создаёт GitHub Release. Итоговые tag/release URL находятся в
evidence register.
